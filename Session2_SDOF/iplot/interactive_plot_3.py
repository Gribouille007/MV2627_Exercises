...
def update(val):
    ...
    # Set y-data for time-domain plot
    line.set_ydata(response)
    
    # Set the single value for red_dot as a sequence (wrap it in a list)
    red_dot.set_ydata([np.abs(fr_val)])
    red_dot.set_xdata([f_slider.val])
    
    # Adjust plot limits
    axes[0].set_xlim([0, 100])
    axes[0].set_ylim([response.min(), response.max()])
